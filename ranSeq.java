...
public class Seq {
	...
	public static String generateRandomSequence(char[] alphabet, float[] weights, int length) throws Exception 
	
		{
			...
			//checks that the round off error is correct
			...
			if(alphabet.length != weights.length) throw new Exception ("lengths of alphabet and weights are not equal");
...
if (sum < 0) 
			{
			    ...
			    if((sum - 1) > 0.001)
			    {
			    	throw new Exception("sum of weights is not within round off error of 1");
			    }
			} 
			else
			{
